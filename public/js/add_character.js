...
let addCharacterForm = document.getElementById('add-character');
...
addCharacterForm.addEventListener("submit", function (e) {
    ...
    let inputCharacterLegal = document.getElementById("input-character_legal");
    let inputCharacterGame = document.getElementById("input-character_game");
    

    // Get the values from the form fields
    let characterNameValue = inputCharacterName.value;
    let characterLegalValue = inputCharacterLegal.value;
    let characterGame = inputCharacterGame.value;
    

    // Put our data we want to send in a javascript object
    let data = {
        character_name: characterNameValue,
        is_legal: characterLegalValue,
        character_game: characterGame,
    ...
    xhttp.onreadystatechange = () => {
        if (xhttp.readyState == 4 && xhttp.status == 200) {
            ...
            inputCharacterLegal.value = '';
            inputCharacterGame.value = '';
...
addRowToTable = (data) => {
    ...
    let characterLegalCell = document.createElement("TD");
    let characterGameCell = document.createElement("TD");
    ...
    deleteCell.onclick = function(){
        deleteCharacter(newRow.id);
    };

    // Add the cells to the row 
    row.appendChild(idCell);
    row.appendChild(characterNameCell);
    row.appendChild(characterLegalCell);
    row.appendChild(characterGameCell);
